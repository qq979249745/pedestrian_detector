...
from detector.utils.google_utils import *
...
def create_modules(module_defs, img_size, arc):
    ...
    hyperparams = module_defs.pop(0)  # 拿到net层
    ...
    for i, mdef in enumerate(module_defs):
        ...
        if mdef['type'] == 'convolutional':
            ...
            if ONNX_EXPORT:  # 显式声明大小，避免scale_factor
                g = (yolo_index + 1) * 2 / 32  # gain
                modules = nn.Upsample(size=tuple(int(x * g) for x in img_size))  # img_size = (320, 192)
            else:
                modules = nn.Upsample(scale_factor=mdef['stride'])

        elif mdef['type'] == 'route':  # nn.Sequential() 占位符 for 'route' layer
            layers = mdef['layers']
...
class Darknet(nn.Module):
    ...
    def __init__(self, cfg, img_size=(416, 416), arc='default'):
        ...
        self.version = np.array([0, 2, 5], dtype=np.int32)  # (int32) 版本信息: major, minor, revision
        self.seen = np.array([0], dtype=np.int64)  # (int64) 在训练中看到的图像的数量
    ...
    def forward(self, x, var=None):
        ...
        for i, (mdef, module) in enumerate(zip(self.module_defs, self.module_list)):
            mtype = mdef['type']
            if mtype in ['convolutional', 'upsample', 'maxpool', 'se', 'cbam']:
                x = module(x)
