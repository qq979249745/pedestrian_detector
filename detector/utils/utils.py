...
def compute_loss(p, targets, model):  # 预测,目标,模型
    ...
    • 最后一个是类别带来的误差，也就是 class 带来的 loss
    """
    ft = torch.cuda.FloatTensor if p[0].is_cuda else torch.Tensor
    lcls, lbox, lobj = ft([0]), ft([0]), ft([0])
    tcls, tbox, indices, anchor_vec = build_targets(model, targets)
    h = model.hyp  # 超参数
    red = 'mean'  # 损失减少 (sum or mean)
...
def plot_results(start=0, stop=0, bucket='', id=()):  # from utils.utils import *; plot_results()
    ...
    s = ['GIoU', 'Objectness', 'Precision', 'Recall',
         'val GIoU', 'val Objectness', 'ACC', 'F1']
    if bucket:
        os.system('rm -rf storage.googleapis.com')
        files = ['https://storage.googleapis.com/%s/results%g.txt' % (bucket, x) for x in id]
    else:
        files = glob.glob('YOLO*.txt') + glob.glob('../../Downloads/results*.txt')
    for f in sorted(files):
        ...
        for i in range(8):
            ...
            ax[i].set_title(s[i])
            if i in []:  # share train and val loss y axes
...
def plot_results1(start=0, stop=0, bucket='', id=()):  # from utils.utils import *; plot_results()
    # Plot training 'results*.txt' as seen in https://github.com/ultralytics/yolov3#training
